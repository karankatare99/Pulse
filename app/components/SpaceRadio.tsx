...
// Define a type for our star data to keep TypeScript happy
type StarData = {
  id: number;
  top: string;
  left: string;
...
const SpaceRadio = () => {
  ...
  // FIX: Generate random stars ONLY on the client side after mount
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-slate-950 overflow-hidden relative">
      ...
      <div className="absolute inset-0 z-0">
        ...
        {/* Floating Stars (Mapped from State to prevent Hydration Error) */}
        {stars.map((star) => (
          <motion.div
            key={star.id}
        ...
        <div className="relative bg-slate-900/60 backdrop-blur-2xl border border-white/10 rounded-[3rem] p-6 shadow-2xl text-white">
          ...
          <div className="text-center mb-8 space-y-1">
            <motion.h2 
              ...
              className="text-xl font-bold bg-clip-text text-transparent bg-linear-to-r from-white to-slate-400"
            >
              {track.title}
            </motion.h2>
          ...
          <div className="flex items-center justify-center gap-6 mb-2">
            ...
            <motion.button
              ...
              className="w-16 h-16 flex items-center justify-center rounded-full bg-linear-to-tr from-cyan-500 to-blue-600 shadow-lg shadow-cyan-500/30 text-white z-20"
            >
              <AnimatePresence mode="wait">
                {isPlaying ? (
                  ...
                  <motion.div
                    ...
                    animate={{ opacity: 1, scale: 1 }}
                    exit={{ opacity: 0, scale: 0.5 }}
                    className="ml-1" // Optical centering for play icon
                  >
          ...
          {/* Scrubber / Progress Bar (Visual only) */}
